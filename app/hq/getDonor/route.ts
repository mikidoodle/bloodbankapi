...
export async function POST(
  ...
  if(token === `hq-${envCode}`) {
    ...
    let donor = await getData(`SELECT name,phone,bloodtype,lastdonated,totaldonated,age,verified FROM users WHERE uuid = '${uuid}';`)
    if (donor.length === 0) {
      return Response.json({ error: true, message: "Donor not found" })
    } else {
      return Response.json({ error: false, message: "Donor found", data: donor[0] })
    }
  } else {
