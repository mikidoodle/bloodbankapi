...
export async function POST(
  ...
  if(token === `hq-${envCode}`) {
    ...
    let minimumDate = new Date(now.getFullYear(), now.getMonth() - parseInt(months), now.getDate())
    let donors = await getData(`SELECT name, notification FROM users WHERE bloodtype = '${type}' AND (lastdonated < '${minimumDate.toISOString()}'
     OR lastdonated IS NULL);`)
    if (donors.length === 0) {
      ...
      for (let notificationobj of donors) {
        ...
        messages.push({
          to: pushToken,
          subtitle: `Blood Center requires ${units} unit${units == 1 ? '' : 's'} of ${type} blood.`,
      ...
      return Response.json({
        error: false,
        message: `Identified and notified ${donors.length} donor${donors.length == 1 ? '' : 's'}.`,
      })
    }
  } else {
