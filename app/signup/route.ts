...
export async function POST( 
  ...
  if (!request.phonenumber || !request.password || !request.name || !request.weight || !request.height || !request.age || !request.bloodtype) {
    return Response.json({ error: true, message: `missing ${
      !request.phonenumber ? 'phonenumber' : !request.password ? 'password' : !request.name ? 'name' : !request.weight ? 'weight' : !request.height ? 'height' : !request.age ? 'age' : 'bloodtype'
    }` })
